...
# - The script will then delete all associated snapshots taken by the script that are older than 28 days
#
...
# How many days do you wish to retain backups for? Default: 28 days
retention_days="28"
...
snapshot_volumes() {
	for volume_id in $volume_list; do
		...
		snapshot_description="$volume_id-$device_name-backup-$(date +%Y-%m-%d)"
...
# Grab all volume IDs attached to this instance whith special tags
volume_list=$(aws ec2 describe-volumes --region $region --query Volumes[].VolumeId --filters "Name=tag:$TAG_KEY,Values=$TAG_VALUE" --output text)
